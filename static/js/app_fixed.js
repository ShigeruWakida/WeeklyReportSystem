...
async function processWeeklyReports() {
    const button = document.getElementById('processReportsBtn');
    const status = document.getElementById('processStatus');
    
    if (button.disabled) return; // 既に処理中の場合は何もしない
    ...
    // ログモーダルを表示
    const modal = new bootstrap.Modal(document.getElementById('processLogModal'));
    modal.show();
    
    try {
        // 処理開始API呼び出し
        const response = await fetch('/api/start_process_reports', {
            method: 'POST'
        });
        ...
        // ログをポーリングで取得
        pollProcessLog(processId);
        
    } catch (error) {
        console.error('週報処理エラー:', error);
        updateLogContent('エラー: ' + error.message, true);
        finishProcessDisplay(false);
    }
}

// ログをポーリングで取得
async function pollProcessLog(processId) {
    let isComplete = false;
    let logOffset = 0;
    
    while (!isComplete) {
        try {
            const response = await fetch(`/api/process_status/${processId}?offset=${logOffset}`);
            const data = await response.json();
            
            if (data.new_output) {
                appendLogContent(data.new_output);
                logOffset = data.current_offset;
            }
            
            if (data.is_complete) {
                isComplete = true;
                finishProcessDisplay(data.success, data.new_reports, data.total_reports);
            } else {
                // 1秒待機してから再度ポーリング
            ...
            updateLogContent('\n[エラー] ログの取得に失敗しました: ' + error.message, true);
            isComplete = true;
            finishProcessDisplay(false);
        }
    }
...
// ログ内容を更新
function updateLogContent(content, isError = false) {
    const logContent = document.getElementById('processLogContent');
    if (isError) {
        logContent.innerHTML = `<div class="text-danger fw-bold">${content}</div>`;
    } else {
        logContent.innerHTML = `<pre class="text-light mb-0" style="color: #ffffff !important;">${content}</pre>`;
    }
...
// 処理完了時の表示更新
function finishProcessDisplay(success, newReports = 0, totalReports = 0) {
    const button = document.getElementById('processReportsBtn');
    const status = document.getElementById('processStatus');
    const closeBtn = document.getElementById('processLogCloseBtn');
    const okBtn = document.getElementById('processLogOkBtn');
    const statusDiv = document.getElementById('processLogStatus');
    
    button.disabled = false;
    button.innerHTML = '<i class="bi bi-envelope-plus"></i> 週報処理';
    ...
    if (success) {
        statusDiv.textContent = `処理完了 - 新規追加: ${newReports}件、合計: ${totalReports}件`;
        statusDiv.className = 'text-success me-auto';
        // レポート一覧を再読み込み
        loadReports();
    } else {
        statusDiv.textContent = '処理エラー';
        statusDiv.className = 'text-danger me-auto';
    }
}
